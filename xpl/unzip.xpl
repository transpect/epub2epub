...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" 
  ...
  <p:try name="try-unzip">
    <p:group>
      ...
      <cx:message>
        <p:with-option name="message" select="'[info] exclude files: ', $remove-files-list"/>
      </cx:message>
      
      <p:xslt name="remove-files-list" cx:depends-on="unzip-epub">
        <p:input port="stylesheet">
          <p:inline>
            <xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
              ...
              <xsl:template match="/c:files/c:file[@name = tokenize($remove-files-list, '\s')]"/>
